...
@section('content')
	...
	<div class="row">
      ...
      <a data-toggle="tooltip" data-placement="top" title="Back" href="{{ url()->previous() }}" class="btn btn-primary"><i class="fa fa-arrow-circle-left" aria-hidden="true"></i></a>
			...
			<form class="d-inline" action="{{route('admin.category.reports.download')}}" method="GET">
   ...
   <div class="col-12">
     ...
     <div class="row">
      <div class="col-12 text-center">
        <h2 class="font-pt font-25">Categories Report</h2>
      </div>
     </div>
     <hr class="mb-3">
     <form action="{{route('admin.reports.category')}}" method="GET">
       <div class="row">
        <div class="col-11">
      ...
      <tbody>
        ...
        if($category->id == 1){
         ...
         <tr align="center">
           ...
           <td class="font-pt font-18"><a target="_blank" class="text-white" href="">{{$category->name}}</a></td>
           <td class="font-pt font-18"><img width="40px" class="" src="{{URL::asset('/assets/img/category')}}/{{$category->image}} " alt="{{$category->name}}"></td>
           <td class="font-pt font-18">{{$category->products->count()}}</td>
