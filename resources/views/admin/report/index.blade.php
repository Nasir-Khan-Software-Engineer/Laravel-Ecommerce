...
<script>
	...
	function makeOrderTable(orders){
		...
		var total_confirm_order 	= 0;
		var total_confirm_product 	= 0;
		var total_confirm_tk 		= 0;

		var total_pending_order 	= 0;
		var total_pending_product 	= 0;
		var total_pending_tk 		= 0;
		...
		for(var i=0;i<orders.length;i++){
			...
			total_tk 		+= order.total_cost;

			if(order.status == 'confirm'){
				total_confirm_order++; 
				total_confirm_product 	+= order.total_product;
				total_confirm_tk 		+= order.total_cost;
			}else{
				total_pending_order++; 
				total_pending_product 	+= order.total_product;
				total_pending_tk 		+= order.total_cost;
			}
		...
		$("#show_total_product").html('Product: '+total_product);
		$("#show_total_tk").html('Total: '+total_tk+ ' Tk');

		$("#show_total_pending_order").html(total_pending_order);
		$("#pending_tk").html('Total: '+total_pending_tk+ ' Tk');
		$("#pending_total_products").html('Product: '+total_pending_product);

		$("#show_total_confirm_order").html(total_confirm_order);
		$("#confirm_tk").html('Total: '+total_confirm_tk + ' Tk');
		$("#confirm_total_products").html('Product: '+total_confirm_product);
