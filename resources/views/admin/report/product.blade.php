...
@section('content')
	...
	<div class="row">
      ...
      <a data-toggle="tooltip" data-placement="top" title="Back" href="{{ url()->previous() }}" class="btn btn-primary"><i class="fa fa-arrow-circle-left" aria-hidden="true"></i></a>
			...
			<form class="d-inline" action="{{route('admin.product.reports.download')}}" method="GET">
   ...
   <div class="col-12">
     ...
     <div class="row">
      <div class="col-12 text-center">
        <h2 class="font-pt font-25">Products Report</h2>
      </div>
     </div>
     <hr class="mb-3">
     <form action="{{route('admin.reports.product')}}" method="GET">
       <div class="row">
        <div class="col-11">
     ...
     <table class="table table-striped table-dark display " id="dataTable">
       <thead>
         <tr align="center">
           ...
           <th scope="col">Total Sell (Qty)</th>
      ...
      <tbody>
		...
		@foreach($products_data as $product)
         ...
         <tr align="center">
           ...
           <td class="font-pt font-18"><a target="_blank" class="text-white" href="{{route('admin.product.show',['slug' => $product['slug']])}}">{{$product['name']}}</a></td>
           ...
           <td class="font-pt font-18">{{$product['total_sell']}} {{$product['unit']}}</td>
