  ...
  <div class="row py-3">
    <div class="col-12 ">
     ...
     <form class="d-inline" action="{{route('admin.order.download')}}" method="POST">
       @csrf
       <button data-toggle="tooltip" data-placement="top" title="Donwload Orders Data" class="btn btn-success"><i class="fa fa-download" aria-hidden="true"></i></button>
     </form>
      ...
      <tbody>
		...
		@foreach($orders as $order)
         ...
         <tr align="center">
           ...
           <td class="font-pt font-18 @if($order->seen == 0) text-danger @endif">{{$order->order_code}}</td>
           <td class="font-pt font-18">{{$order->created_at}}</td>
